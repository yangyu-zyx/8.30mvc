...
function animate(obj, attrObj, duration, fn, callback) {// 对象  属性  时间  动画的函数
    // 参数初始化
    ...
    var start = {};  //存储所有属性开始的值
    var change = {};   //变化值
    var time = 0;       //当前运行时间
    var fn = fn || Tween.Linear;  //动画运行方式



    // 获取每个属性的初始值
    for (var i in attrObj) {
        start[i] = parseInt(getComputedStyle(obj, null)[i]);//i是属性
        change[i] = attrObj[i] - start[i];//变化
    }
